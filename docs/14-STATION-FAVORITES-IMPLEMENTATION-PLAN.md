...
数据库变更必须先按 `docs/04-DATABASE.md` 的公共契约流程评审。本功能建议新增一张关联表：
...
消息名、字段和错误码最终必须同步到 `docs/03-API.md` 与 `shared/protocol`。
...
为避免列表上的收藏按钮还要逐项请求，建议同时扩展已登录用户的站点响应：
...
1. 收藏按钮采用星标还是爱心（建议：星标，和“喜欢/点赞”语义区分）；
2. 收藏列表是否按“最近收藏”还是“距当前位置最近”排序（建议：默认最近收藏，提供按距离排序）；
3. 管理端是否需要查看某站点收藏人数（建议：首版不做，只预留后续统计可能）。
