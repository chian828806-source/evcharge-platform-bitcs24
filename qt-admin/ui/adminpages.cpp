...
void PilePage::applyFilter()
{
    ...
    m_table->clear(); m_table->clearSpans(); m_table->setRowCount(piles.size()); m_table->setColumnCount(10);
    m_table->setHorizontalHeaderLabels({QStringLiteral("桩号"), QStringLiteral("站点"), QStringLiteral("类型"), QStringLiteral("额定功率"), QStringLiteral("状态"), QStringLiteral("实时功率"), QStringLiteral("温度/故障"), QStringLiteral("次数"), QStringLiteral("分钟"), QStringLiteral("操作")});
    if (piles.isEmpty()) {
        showEmptyState(m_table, 10, wanted.isEmpty() ? QStringLiteral("暂无电桩")
    ...
    for (int row = 0; row < piles.size(); ++row) {
        ...
        auto *restart = tableActionButton(m_table, row, 9, QStringLiteral("远程重启"));
    ...
    configureActionColumn(m_table, 9);
}
