...
--    默认账号 admin/123456，只存 PBKDF2-HMAC-SHA256 派生值；Socket 响应禁止返回 password_hash
-- ----------------------------------------------------------------------------
CREATE TABLE admin (
...
CREATE TABLE charging_station (
    ...
    CHECK (service_fee_fen_per_kwh >= 0),
...
CREATE TABLE charging_pile (
    ...
    CHECK (power_kw > 0),
    CHECK (total_charge_count >= 0),
    CHECK (total_charge_minutes >= 0),
...
CREATE TABLE prediction (
    ...
    CHECK (predicted_available_count >= 0),
    CHECK (mae >= 0),
    CHECK (rmse >= 0),
