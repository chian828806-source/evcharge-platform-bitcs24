...
真实模式已由当前 `DashboardDataService` 完成数据闭环：

- `summary`：复用 `OrderRepository` 的营收、今日完成电量和已完成订单汇总，并按
  `RESERVED + CHARGING` / 总桩数计算当前负荷；
- `pileStatus`：复用 `PileRepository::statusSummary()`；
- `revenueTrend`：复用既有 `OrderRepository::revenueTrend()`，一次 update 同时发送
  `7d` 和 `30d`；
- `prediction`：复用 `PredictionService::list()` 与当前 `PREDICTION_LIST` 的 `items`
  形状，因此 ML 导入后的数据会直接进入 Dashboard。

`DashboardWebSocketServer` 仍只负责路径、订阅和推送，不直接访问 SQLite。每次
`DASHBOARD_SUBSCRIBE` 成功后都会按订阅 topic 返回 initial snapshot；服务端随后以
3 秒只读刷新发布当前状态，覆盖订单、站点、电桩和 ML 导入造成的业务变化。Mock
模式仍只使用 `mock/dashboard-mock.js`，不会连接或修改真实 Server。

仍待真实部署环境确认的仅是浏览器与 Ubuntu Qt 6.2.4 Server 的端到端连通性及
生产级推送频率；不需要为此修改公共协议或复制业务 SQL。
...
浏览器不需要结构性重写：服务端按本文档数据形状推送四个既有 topic，断线时保留
最后一次有效数据，重连订阅后以新的 initial snapshot 恢复。
